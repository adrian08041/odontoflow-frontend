...
type AgendaNewDialogStepperProps = {
  step: number;
  totalSteps: number;
  stepLabels: string[];
  onClose: () => void;
...
export function AgendaNewDialogStepper({
  step,
  totalSteps,
  stepLabels,
  onClose,
}: AgendaNewDialogStepperProps) {
  return (
    <div className="bg-[linear-gradient(135deg,var(--color-brand-teal)_0%,var(--color-brand-teal-soft)_100%)] px-4 py-6 text-white sm:px-6 sm:py-7 md:px-8">
      ...
      <div className="mt-6 flex items-center justify-center gap-2 text-center sm:mt-8 sm:gap-3">
        {Array.from({ length: totalSteps }, (_, index) => index + 1).map((item, index) => (
